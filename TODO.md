...
- [x] Python 버전 정책 적용: `3.14 -> 3.13 -> 3.12` 단계 하향
- [x] WebUI 스택은 확장 단계에서 `FastAPI` 사용
- [x] 운영 우선순위는 `CUDA` 고정 (ROCm/Intel은 Phase 4 이후)
- [x] MVP 출력 포맷은 `SRT` 단일
- [x] 번역 인터페이스는 OpenAI-Compatible 표준 사용
...
- [x] Qwen3-ASR-1.7B 연동
- [x] Qwen3-ForcedAligner-0.6B 연동
- [x] 파이프라인 연결 (`VAD -> ASR -> Align -> Subtitle`)
- [x] SRT writer 구현
- [x] 메타데이터/로그 출력 (`segments.json`, `run.json`, `*.log`)
- [x] `transcribe` 단일 파일 E2E 검증
- [x] `batch` 다중 파일 E2E 검증
- [x] 샘플 파일 2개(짧은/긴) 기준 RTF 측정
- [ ] 샘플 파일 2개(짧은/긴) 기준 CER/WER 측정 (성능/품질 벤치마크는 Phase 4 이후 재개)

## Phase 1 완료 기준

- [x] 로컬에서 CLI 한 줄 명령으로 SRT 생성 가능
- [x] 실패 시 원인 파악 가능한 로그 출력
- [x] 동일 입력 재실행 시 결과 재현 가능
...
- [ ] 성능/품질 벤치마크 재개 (CER/WER/RTF 재측정, CUDA 환경 기준선 재작성)
